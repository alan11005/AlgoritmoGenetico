...
class Genotipo:
    ...
    def __init__(self, genes, bits, utilidad, restriccion, arrTamaños, arrXmax, arrXmin, arrNdecimales, genotipo=None):
        ...
        self.Z = self.calcularZ()
        self.R = self.calcularR(restriccion)
    ...
    def calcularZ(self):
        #Calcular el Z para la funcion sen(x) * sen(y) + 25 donde fenotipo
        #es un array con los valores de x y y
        x = self.getFenotipo()[0]
        y = self.getFenotipo()[1]
        return np.sin(2*x) * 4* np.cos(y)
